...
The COVID-19 pandemic caused major disruption in the supply chain. The world has since felt the negative impact of such disruption on trade and the global economy, forcing us to think about the other sources of threats to maritime trade like piracy. Maritime piracy events have been increasing worldwide since the 1990s and has since ebbed and flowed. For instance, the first wave of piracy in the Southeast Asia resulted after the Asian financial crisis in 1997 and the end of Suharto regime in Indonesia a year later. In this paper, we examine the conditions created by the current pandemic to explore how poverty affects maritime piracy. 
  
  There have been a number of studies on the impact of COVID-19 on crime on land. But to our knowledge, there is a lack of systematic and empirical study of the pandemic on crime in sea. In this article, we take what is known about how COVID-19 devastated the global economy to investigate how the pandemic affected the level of maritime piracy among littoral states. Our aim is to situate this study on piracy in the larger literature on the relationship between crime and the economy, how they affect each other, whether the relationship is possibly endogenous, and if so, how best to capture it.
...
Past works suggest various factors contribute to increase in piracy. According to \citet{hastings2009geographies}, a country’s weak governance and infrastructure leads to increased piracy in its waters. Others focus on opportunity factors like a country’s weak governance capacity. Studies suggest that a country with weak infrastructure but situated near shipping traffic lanes or choke-points are more likely to experience piracy events near it \citep{prins2017maritime}. The rise of piracy in the Gulf of Aden and Somali waters since the 2000s directly contributed to this argument.   Yet, \citet{daxecker2013insurgents} argue that policymakers should also focus on the root causes. According to the authors, while weak governance and target-rich environments lower the costs for potential pirates, the root causes like poverty and unemployment drive individuals more towards such activities. To study whether economic motivation drives piracy events, \citet{Tominaga2018} looks at the fluctuations in the fishing  market to predict its impact on piracy. 

Past studies use panel and aggregate data to show how permissive institutional environment can lead to increased piracy \citep{daxecker2013insurgents}. However, findings for the root causes or the economic drivers of piracy is still unclear.  \citet{daxecker2013insurgents} use aggregate country and yearly variables to show link between fluctuations in the fish values with piracy. They show that as a country’s lowered fish values is associated with increased piracy. However, as \citep[39]{Tominaga2018} indicates, the correlation may be endogenous. There could be reverse causality, that increased piracy leading to less fishing. Alternatively, an increase in piracy events could make the governments recognize the necessity of investing more in the infrastructure of fisheries. Citing these shortcomings, \citet{Tominaga2018} uses instrumental variable approach to estimate the effect of fish value on piracy, and his empirical results provide only partial supportive evidence for his claims, casting considerable doubt on the causal claim about root causes of piracy. Despite the authors earlier critique, he too uses the aggregate cross-country and yearly-level observational dataset. 
...
\begin{figure}[h]
```
```{r pcount,  fig.show="hold", out.width="48%", echo=FALSE}
par(mar = c(4, 4, .1, .1))

knitr::include_graphics(here::here("figures/yearly_piracy_1993-2021.png"))
knitr::include_graphics(here::here("figures/yearly_piracy_loess_1993-2021.png"))
```
```{=latex}
...
knitr::include_graphics(here::here("figures/cumulative_restriction_indo_nigeria.png"))
...
Table 1 shows the results of interrupted time series analysis comparing trends pre- and post-COVID-induced stringency in Nigeria and Indonesia. For comparative purpose, we use data after the start of the year 2020, around the time when COVID-19 pandemic started to be seen, until June 2021. Intervention in the two countries are the months when monthly cumulative restriction peak in the countries, which are April 2020 for Nigeria and December 2020 for Indonesia, as shown in Figure \ref{fig:stringency}. We start by estimating single-group ITSA using the ITSA package written for Stata. We then estimate multi-group ITSA, where we include three neighboring countries as controls. For Nigeria, we include nearby Ghana, Sierra Leone and Ghana, and for Indonesia, we include Malaysia, Thailand and Philippines. Newey-West standard errors were specified to account for autocorrelation at lag 1. In the table, models (1) and (3) are single-group ITSA models for Nigeria and Indonesia respectively. Models (2) and (4) are multi-group ITSA models for the two countries including the neghbouring countries. Variable of interests are x_t_Country for single group models and _z_x_t_Country for multi-group models. These variables are positive for all four models, indicating a positive piracy trend in the aftermath of severe restrictions. However, we find statistical significance only for Nigeria. 
...
\hline\hline
                    ...
                    &\multicolumn{1}{c}{Nigeria}&\multicolumn{1}{c}{Nigeria}&\multicolumn{1}{c}{Indonesia}&\multicolumn{1}{c}{Indonesia}\\
...
\_z\_x_Nigeria             &           &   2.008   &           &           \\
                    &           & (1.960)   &           &           \\
\_z\_x\_t_Nigeria           &           &   2.456***&           &           \\
                    &           & (0.801)   &           &           \\
\_x_Indoneisia               &           &           &  -0.456   &  -0.609   \\
                    &           &           & (2.775)   & (0.578)   \\
\_x\_t_Indoneisia            &           &           &   0.466   &   0.164   \\
                    &           &           & (0.533)   & (0.149)   \\
\_z\_x_Indoneisia             &           &           &           &   0.152   \\
                    &           &           &           & (2.659)   \\
\_z\_x\_t_Indoneisia          &           &           &           &   0.302   \\
                    &           &           &           & (0.520)   \\
...
```{r result1,  fig.show="hold", out.width="48%", echo=FALSE, fig.cap.align='center', fig.align="center"}
par(mar = c(4, 4, .1, .1))

knitr::include_graphics(here::here("figures/nigeria_single.png"))
knitr::include_graphics(here::here("figures/nigeria_multi.png"))

knitr::include_graphics(here::here("figures/indonesia_single.png"))
knitr::include_graphics(here::here("figures/indonesia_multi.png"))

```
```{=latex}
\caption{Stringency measures in response to COVID-19 \label{fig:nigeria}}
\textit{Note: Panels in the figure above are from ITSA models in Table 2. They show patterns of piracy after highest monthly cumulative restrictions imposed by the government in Indonesia and Nigeria.}
\end{figure}
```
